...
├── workshop/
│   └── cdss_kit.py                     Carries code between notebooks
├── notebooks/
...
| [![Colab](https://img.shields.io/badge/Colab-F9AB00?logo=googlecolab&logoColor=white)](https://colab.research.google.com/github/utkukose/cdss-genai-NB-lecture/blob/main/notebooks/en/NB3_Training_and_Evaluation.ipynb) | **NB3** Training and evaluation | Model, predictions and a nine value honest measurement |
| [![Colab](https://img.shields.io/badge/Colab-F9AB00?logo=googlecolab&logoColor=white)](https://colab.research.google.com/github/utkukose/cdss-genai-NB-lecture/blob/main/notebooks/en/NB4_Explaining_the_Decision.ipynb) | **NB4** Explaining the decision | Overall and per patient reasoning, and its critique |
...
short prompt written in plain clinical language. You pass it to a generative AI tool, paste
the code it returns into the blank cell below the prompt, and run it.

A few questions follow each step. Answer them by reading your code and its output. Those
questions are the substance of the workshop: generated code looks right at first glance, and
so do the places where it is wrong. The questions target leakage, the patient level split,
class imbalance and the regulatory dates directly.

The notebooks run no automatic code checks. Verification is yours, and that is what the
workshop teaches.
...
One module, `workshop/cdss_kit.py`. It only carries the code forward. It contains no clinical decision support logic; that is what you write.
