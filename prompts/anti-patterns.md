...
Sıfırıncı istem, yani sorgulama istemi, bu listedeki hataları yakalamak için yazılmıştır.
Liste, o istemin neden o soruları sorduğunu açıklamaktadır.
...
**Nasıl yakalanır.** Sorgulama isteminin birinci maddesi her sayının kaynağını sormaktadır.
Kaynağı olmayan değerler kodda `KAYNAKSIZ` olarak işaretlenmekte ve klinik olarak
doğrulanana kadar öyle kalmaktadır.
...
olarak imkânsız hâle gelmektedir. Sorgulama isteminin ikinci maddesi bu yolu açıkça
izletmektedir.
...
var mıydı? Sorgulama isteminin üçüncü maddesi bunu sormaktadır. Atölyenin veri modülünde
...
**Nasıl yakalanır.** Sorgulama isteminin beşinci maddesi her atıf için DOI ya da resmî
belge numarası istemektedir. DOI verilemiyorsa atıf kaldırılmaktadır. Verilen DOI ise
...
**Nasıl yakalanır.** Yedinci istemin çıktısına sorgulama istemi özellikle dikkatli
uygulanır. Her düzenleyici iddia için güven düzeyi ve bir hukukçunun kontrol etmesi
gereken nokta ayrıca istenir.
...
1. Sorgulama istemini çalıştırınız.
2. Birinci maddedeki sayı listesini okuyunuz. Kaynaksız olanları işaretleyiniz.
...
Altıncı adımın sonda olması bilinçlidir. Sonuçlara önce bakıldığında, iyi görünen bir
sayının arkasındaki hatayı aramak zorlaşmaktadır.
