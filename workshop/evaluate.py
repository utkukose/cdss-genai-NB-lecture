...
_W = {
    "head":      {"tr": "DÜRÜST DEĞERLENDİRME", "en": "HONEST EVALUATION"},
    "testset":   {"tr": "Test kümesi: {} vaka, {} pozitif ({:.1%})",
                  "en": "Test set: {} cases, {} positive ({:.1%})"},
    "warn_few":  {"tr": ("UYARI. Test kümesinde yirmi beşten az pozitif vaka bulunmaktadır.\n"
                         "Aşağıdaki değerlerin tamamı yöntemin gösterimi niteliğindedir ve\n"
                         "başarım tahmini olarak okunamaz. Nokta tahmine değil güven aralığına\n"
                         "bakınız."),
                  "en": ("WARNING. Fewer than twenty five positive cases in the test set.\n"
                         "Every figure below illustrates the method and cannot be read as an\n"
                         "estimate of performance. Read the confidence interval, not the point\n"
                         "estimate.")},
    "s1":        {"tr": "1. AYRIM GÜCÜ", "en": "1. DISCRIMINATION"},
    "noskill":   {"tr": "beceriksiz model değeri", "en": "no-skill value"},
    "chance":    {"tr": "   Aralık 0,5 değerini içeriyor. Model şanstan ayırt edilemiyor.",
                  "en": "   The interval includes 0.5, so the model is not distinguishable from chance."},
    "s2":        {"tr": "2. KALİBRASYON", "en": "2. CALIBRATION"},
    "verdict":   {"tr": "   Değerlendirme: {}", "en": "   Verdict: {}"},
    "s3":        {"tr": "3. ÇALIŞMA NOKTASI", "en": "3. OPERATING POINT"},
    "thr":       {"tr": "   Duyarlılık >= {:.0%} için seçilen eşik: {:.3f}",
                  "en": "   Threshold chosen for sensitivity >= {:.0%}: {:.3f}"},
    "sens":      {"tr": "   Duyarlılık {:.3f}   Özgüllük {:.3f}",
                  "en": "   Sensitivity {:.3f}   Specificity {:.3f}"},
    "ppv":       {"tr": "   PKD         {:.3f}   NKD         {:.3f}",
                  "en": "   PPV         {:.3f}   NPV         {:.3f}"},
    "counts":    {"tr": "   Sayılar: DP {}, YP {}, YN {}, DN {}",
                  "en": "   Counts: TP {}, FP {}, FN {}, TN {}"},
    "s4":        {"tr": "4. KLİNİK KARŞILIK", "en": "4. CLINICAL TRANSLATION"},
    "shift":     {"tr": "   Her 100 hastada {} uyarı çıkar; {} doğru, {} yanlış.",
                  "en": "   Per 100 patients: {} alerts fire, {} true, {} false."},
    "missed":    {"tr": "   {} vaka tamamen kaçırılır.",
                  "en": "   {} cases are missed entirely."},
    "s5":        {"tr": "5. ALT GRUP DÖKÜMÜ", "en": "5. SUBGROUP BREAKDOWN"},
    "nogroup":   {"tr": "   Gruplama değişkeni verilmedi.",
                  "en": "   No grouping variable supplied."},
    "refused":   {"tr": ("   {} alt grup değerlendirilemeyecek kadar küçük. Bu boşluk bir\n"
                         "   bulgudur: Hiç test edilmemiş bir grup için sistemin adil olduğu\n"
                         "   gösterilemez."),
                  "en": ("   {} subgroup(s) too small to evaluate. That absence is a finding:\n"
                         "   a system cannot be shown to be fair for a group it was never\n"
                         "   tested on.")},
    "s6":        {"tr": "6. BOŞ KARŞILAŞTIRMA", "en": "6. NULL COMPARISON"},
    "nullline":  {"tr": "   {}: doğruluk {:.3f}, AUC {:.2f}",
                  "en": "   {}: accuracy {:.3f}, AUC {:.2f}"},
    "nullnote":  {"tr": "   Bir modelin tartışmaya değer olması için bu değerleri aşması gerekir.",
                  "en": "   Any model must beat these numbers before it is worth discussing."},
    "always":    {"tr": "her zaman {} tahmin etmek", "en": "always predict {}"},
    "cal_over":  {"tr": "aşırı güvenli, tahminler fazla uç değerlerde",
                  "en": "over-confident, predictions are too extreme"},
    "cal_under": {"tr": "yetersiz güvenli, tahminler fazla ortada toplanmış",
                  "en": "under-confident, predictions are too flat"},
    "cal_ok":    {"tr": "bu örneklemde kalibrasyona yakın",
                  "en": "close to well calibrated on this sample"},
}


def _w(key, *args):
    return _W[key][LANG].format(*args)
...
def calibration_report(y_true, y_prob, n_bins: int = 5) -> dict:
    ...
    return {
        ...
        "verdict": (
            _w("cal_over") if slope < 0.9
            else _w("cal_under") if slope > 1.1
            else _w("cal_ok")
        ),
    }
...
def honest_report(y_true, y_prob, groups=None, target_sensitivity: float = 0.80,
    ...
    print("=" * 72)
    print(f"{_w('head')} · {label}")
    print("=" * 72)
    print(_w("testset", n, positives, positives / n))
    if positives < 25:
        print()
        print(_w("warn_few"))
    print()
    ...
    print(f"   ROC AUC            {disc['auc']:.3f}  (95% CI {disc['ci_low']:.3f} - {disc['ci_high']:.3f})")
    print(f"   Average precision  {disc['average_precision']:.3f}  "
          f"({_w('noskill')} {disc['prevalence']:.3f})")
    ...
    print(_w("thr", target_sensitivity, threshold))
    print(_w("sens", point["sensitivity"], point["specificity"]))
    print(_w("ppv", point["ppv"], point["npv"]))
    print(_w("counts", point["tp"], point["fp"], point["fn"], point["tn"]))
    print()

    print(_w("s4"))
    shift = alerts_per_hundred(point["sensitivity"], point["specificity"], disc["prevalence"])
    print(_w("shift", shift["alerts_fired"], shift["true_alerts"], shift["false_alerts"]))
    print(_w("missed", shift["missed_cases"]))
    print()
    ...
    if groups is None:
        ...
        if refused:
            print(_w("refused", refused))
    print()

    print(_w("s6"))
    null = null_comparison(y_true)
    majority = null["strategy"].split()[-1]
    print(_w("nullline", _w("always", majority), null["accuracy"], null["auc"]))
    print(_w("nullnote"))
    print("=" * 72)
